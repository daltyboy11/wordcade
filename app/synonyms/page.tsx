...
const data = [
  ...
  { word: "pragmatic", options: ["idealistic", "practical", "theoretical"], answer: 1 },
];
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-gradient-to-r from-purple-500 to-pink-500 text-white">
      {pageState === "pregame" && (
        <div className="text-center">
          ...
          <p className="text-xl mb-8">
            Choose the correct synonym for each word. You have 30 seconds to answer as many as you can.
          </p>
          <button
      ...
      {pageState === "ingame" && (
        <div className="text-center" style={{ minWidth: "300px"}}>
          <div className="mb-4">
            <h1 className="text-4xl font-bold" style={{ minWidth: "200px" }}>{timeLeft}</h1>
          </div>
              ...
              return (
                <li key={index} className="mb-2">
                  {question.word}: {isCorrect ? (
                    <span className="text-green-500">✅ {question.options[answer]}</span>
                  ) : (
                    <span className="text-orange-300">
                      ❌ {question.options[answer]} (Correct: {question.options[question.answer]})
                    </span>
                  )}
                </li>
              );
